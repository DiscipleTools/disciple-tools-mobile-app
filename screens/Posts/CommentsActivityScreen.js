...
const updateCommentComponent = ({ items, setItems, comment, commentId }) => {
  const existingCommentIdx = items?.findIndex(
    (item) => item?.ID === commentId || item?.tmpId === commentId
  );
...
const deleteCommentComponent = ({ items, setItems, commentId }) =>
  setItems(
    items.filter(
      (item) => item?.comment_ID !== commentId && item?.tmpId !== commentId
    )
  );
...
const updateCommentCache = ({ cache, cacheKey, commentId, comment }) => {
  ...
  const existingCommentIdx = cachedData.comments?.findIndex(
    (item) => item?.comment_ID === commentId || item?.tmpId === commentId
  );
...
const deleteCommentCache = ({ cache, cacheKey, commentId }) => {
  const cachedData = cache.get(cacheKey);
  if (!cachedData?.comments) return;
  cachedData.comments = cachedData.comments.filter(
    (item) => item?.comment_ID !== commentId && item?.tmpId !== commentId
  );
  cache.set(cacheKey, cachedData);
};
...
const CommentInput = forwardRef(
  (
    ...
    const onSave = async (comment) => {
      ...
      try {
        if (
          (editComment?.id || editComment?.tmpId) &&
          editComment?.message?.length > 0
        ) {
...
const CommentsActivityItem = ({
  ...
  const onEdit = () =>
    setEditComment({ id: item?.comment_ID || item?.tmpId, message });
          ...
          >
            <View>
              <Text style={styles.footerText}>
                {isEditing ? editComment.message : i18n.t("global.comments")}
              </Text>
            </View>
