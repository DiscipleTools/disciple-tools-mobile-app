  ...
  return Object.fromEntries(
    Object.entries(post).filter(([_, val]) => {
      if (Array.isArray(val)) {
        if (val.length === 1 && (val[0]?.value === null || val[0]?.value === '')) {
          return false;
        };
      };
...
const validateFields = ({ fields, post, i18n, toast }) => {
  const requiredFields = fields?.filter(([_, field]) => field?.required === true && field?.in_create_form === true);
...
const mapToAPI = ({ post, postType }) => {
  ...
  if (postType === TypeConstants.CONTACT) {
    ...
    mappedPost[FieldNames.FAITH_STATUS] = post?.[FieldNames.FAITH_STATUS]?.key ?? '';
  };
  if (postType === TypeConstants.GROUP) {
    mappedPost[FieldNames.GROUP_TYPE] = post?.[FieldNames.GROUP_TYPE]?.key ?? '';
  };
  return mappedPost;
};
...
const CreateScreen = ({ navigation, route }) => {
  ...
  const creationFields = fieldEntries?.filter(([_, value]) => value?.in_create_form === true && value?.hidden !== true);
  if (isContact) {
    ...
    */
    if (route?.params?.post?.[FieldNames.CONTACT_PHONE] && !creationFields?.find(([key, _]) => key === FieldNames.CONTACT_PHONE)) {
      creationFields.push([
    ...
    if (route?.params?.post?.[FieldNames.CONTACT_EMAIL] && !creationFields?.find(([key, _]) => key === FieldNames.CONTACT_EMAIL)) {
      creationFields.push([
  ...
  const initialPost = initialMapFieldsToPost({ fields: creationFields, post: route?.params?.post, postType });
