...
const filterPostFields = ({ fields, post }) => fields?.filter(([key, _]) => key in post);

// expects fields in Object.entries(...) format
const filterTileFields = ({ fields, tileKey }) => fields?.filter(([_, field]) => field?.tile === tileKey);
...
const DetailsScreen = ({ navigation, route }) => {
  ...
  const sortedTileEntries = [...tileEntries].sort((a, b) => a[1]?.tile_priority - b[1]?.tile_priority);

  // tab view component routes
  const routes = sortedTileEntries.map(([key, val]) => ({ key, title: val?.label ?? '' }));

  // tab view component render
  const renderScene = ({ route }) => {
    const tileFields = filterTileFields({ fields: fieldEntries, tileKey: route.key })
