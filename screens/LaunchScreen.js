...
const filterActivityLog = ({ activityLog, postType }) => {
  const filteredActivityLog = activityLog.filter(item => item?.post_type === postType);
...
const prefetchListFresh = async({ postType }) => {
  ...
  const activityLog  = await mutate(ActivityLogURL, defaultFetcher({ url: ActivityLogURL, method: "GET" }), options);
  ...
  const missingActivityLogIds = filterActivityLogByMissingPosts({ uniqueActivityLogIds, posts });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={globalStyles.screenContainer}>
        ...
        { isConnected && fetching && !hydrating && <PrefetchData setFetching={setFetching} /> }
