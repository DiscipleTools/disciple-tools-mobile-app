...
export const persistCache = async () => { 
  ...
  if (!isEmpty(cacheObj)) {
    ...
    await FileSystem.writeAsStringAsync(cacheFilename, JSON.stringify(cacheObj), { encoding: FileSystem.EncodingType.UTF8 });
  };
};
...
export const filterPosts = ({ posts, query }) => {
  ...
  return posts?.filter((item) => {
    ...
    queryKeys = queryKeys.filter((key) => key !== "sort" && key !== "combine" && key !== "subassigned");
    ...
    const queryMatches = queryKeys?.map((key) => {
      ...
      if (key === "assigned_to" && query[key][0] === "me" && item?.[key]?.id === userData?.ID?.toString()) return true;
      // special handler for "subassigned" (and other "me") fields
      if (meMultipleFields.includes(key) && query[key][0] === "me" && item?.[key]?.find(_item => _item?.ID === userData?.ID?.toString())) return true;
      return false;
    });
...
export const sortPosts = ({ posts, sortKey }) => {
  ...
  return posts.sort((a, b) => {
    ...
    if (secondarySortKey !== null) {
      if (a?.[sortKey]?.[secondarySortKey] < b?.[sortKey]?.[secondarySortKey]) return sortDesc ? 1 : -1;
      if (a?.[sortKey]?.[secondarySortKey] > b?.[sortKey]?.[secondarySortKey]) return sortDesc ? -1 : 1;
      return 0;
    };
...
export const getDefaultFavoritesFilter = ({ i18n, type } = {}) => {
  ...
  defaultFavoritesFilter["name"] = type === TypeConstants.CONTACT ? i18n.t("global.favoriteContacts") : (type === TypeConstants.GROUP ? i18n.t("global.favoriteGroups") : '');
  return defaultFavoritesFilter;
};

export const defaultFetcher = (request) => async(key) => axios(request).then((res) => res.data);
...
export const downloadAllData = async() => {
  const options = { revalidate: true, rollbackOnError: true };
  // refresh settings
  await mutate(SettingsURL, defaultFetcher({ url: SettingsURL, method: "GET" }), options);
  // refresh my user data
  await mutate(MyUserDataURL, defaultFetcher({ url: MyUserDataURL, method: "GET" }), options); 
  // refresh notifications
  await mutate(NotificationsURL, defaultFetcher(NotificationsRequest), options);
  // refresh users 
  await mutate(UsersURL, defaultFetcher({ url: UsersURL, method: "GET" }), options);
  // refresh group filters
  const groupFiltersURL = getFiltersURL({ postType: TypeConstants.GROUP });
  await mutate(groupFiltersURL, defaultFetcher({ url: groupFiltersURL, method: "GET" }), options);
  // refresh contact filters
  const contactFiltersURL = getFiltersURL({ postType: TypeConstants.CONTACT });
  await mutate(contactFiltersURL, defaultFetcher({ url: contactFiltersURL, method: "GET" }), options);
  // refresh locations
  await mutate(LocationsURL, defaultFetcher({ url: LocationsURL, method: "GET" }), options);
  // refresh people groups
  await mutate(PeopleGroupsURL, defaultFetcher({ url: PeopleGroupsURL, method: "GET" }), options);
  // refresh groups
  const groupsListURL = getListURL({ postType: TypeConstants.GROUP });
  const groups = await mutate(groupsListURL, postFetcher({ url: groupsListURL, offset: 0, limit: 1000, posts: [] }), options);
  // refresh contacts
  const contactsListURL = getListURL({ postType: TypeConstants.CONTACT });
  const contacts = await mutate(contactsListURL, postFetcher({ url: contactsListURL, offset: 0, limit: 1000, posts: [] }), options);
  ...
  await fetchPostData({ postType: TypeConstants.CONTACT, posts: contacts?.posts })
  return;
};
