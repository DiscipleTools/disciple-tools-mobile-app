...
const useCommentsActivity = ({ search, filter, exclude }) => {
  ...
  let { data: comments, error, isLoading, isValidating, mutate } = useComments({ search, exclude });
  const { data: activity, error: errorActivity, isLoading: isLoadingActivity, isValidating: isValidatingActivity } = useActivity({ search, filter, exclude });
  if (isLoadingActivity || isLoading || !comments || !activity) return {
    data: [],
  ...
  if (filter?.query?.key) merged = merged.filter(item => item.hasOwnProperty(filter.query.key));
