...
const useUsersContacts = ({ search, filter, exclude }) => {
  const {
    ...
    mutate,
  } = useUsers({ search, exclude });
  const {
    data: contacts,
    error: errorContacts,
    isLoading: isLoadingContacts,
    isValidating: isValidatingContacts,
  } = useList({ search, filter, exclude, type: TypeConstants.CONTACT });
  if (isLoadingUsers || isLoadingContacts || !users || !contacts)
    return {
      data: null,
