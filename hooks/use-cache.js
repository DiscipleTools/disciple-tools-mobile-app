...
const useCache = () => {
  ...
  const loadCache = async () => {
    const cacheFilename =
      FileSystem.documentDirectory + CacheConstants.FILENAME;
    let cacheObj;
    try {
      const cacheStr = await FileSystem.readAsStringAsync(cacheFilename, {
  ...
  // TODO: remove
  const mergeValueIntoCacheByType = useCallback(
    ({ cachedValue, newValue, fieldType }) => {
  ...
  // TODO: remove
  const mergeValueIntoCacheByKey = ({
    cacheKey,
    fieldKey,
    fieldType,
    newValue,
    ...
    const mergedData = mergeValueIntoCacheByType({
      cachedValue,
      newValue,
      fieldType,
    });
  ...
  const setCacheFilterPostFieldValues = ({
    cacheKey,
    fieldKey,
    filterValue,
    postId,
    ...
    const idx = cachedData?.posts?.findIndex((post) => post.ID === postId);
    if (!cachedData?.posts?.[idx]?.[fieldKey]) return;
    const cachedDataModified = cachedData.posts[idx][fieldKey]?.filter(
  ...
  const clearStorage = useCallback(async () => {
    try {
      ...
      await FileSystem.writeAsStringAsync(cacheFilename, JSON.stringify({}), {
        encoding: FileSystem.EncodingType.UTF8,
      });
    } catch (error) {
      console.error(error);
