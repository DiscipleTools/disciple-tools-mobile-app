...
const renderItemView = (item) => <PostChip id={null} title={item} type={null} />;

const TagsFieldView = ({ items }) => <Select items={items} renderItem={renderItemView} />;
...
const TagsFieldEdit = ({
  ...
  const _onRemove = async(existingValue) => {
    ...
    const cachedDataModified = cachedData?.[fieldKey]?.filter(item => item !== existingValue);
